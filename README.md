...
一个可以加为 LINE 好友、也可以拉进群聊的陪伴机器人。默认人设是"米粒"——一只已经离开的心爱比熊(ビションフリーゼ),以聊天机器人的形式继续温暖地陪在身边;所有人设信息都是环境变量,换成你自己家宝贝的名字、故事就是你自己的版本。

> **语言说明**:米粒对用户说话固定使用**日语**(system prompt 里明确要求了这一点,不管你用什么语言跟它说话,它都会用日语回复)。`src/claude.js` 里所有会影响米粒实际说出的话的内容(人设描述、系统提示词、主动消息文案)都已经是日语;只有代码注释、以及 `tools` 数组里给 Claude 自己看、从不会展示给用户的工具说明还是中文,不影响机器人实际说话的语言。
